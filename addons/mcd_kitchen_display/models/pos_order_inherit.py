...
class PosOrder(models.Model):
    ...
    def _send_to_kitchen(self):
        """Create a Kitchen Display order once the POS order is paid."""
        existing = self.env['kitchen.order'].search([
        ...
        return self.env['kitchen.order'].create({
            'name': display_code,
            'pos_order_id': self.id,
    ...
    def _mcd_prepare_display_lines(self):
        ...
        for line in self.lines:
            if not line.product_id or line.qty <= 0:
                continue
            if self._mcd_skip_kitchen_product(line.product_id):
                continue
    ...
    def _mcd_skip_kitchen_product(self, product):
